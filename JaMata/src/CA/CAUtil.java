...
public class CAUtil 
{
	...
	public static CA removeUnreachable(CA at)
	{
		...
		for (int ind=0;ind<finalTr.length;ind++)
		{
			...
			int[] s = t.getInitialP();
			if(!amIReachable(s,aut,aut.getInitialCA(),new int[aut.sumStates()][],0))
			{
				finalTr[ind]=null;
				removed++;
			}
	 ...
	 */
	public static CA removeHangedTransitions(CA at)
	{
		CA aut = at.clone();
		CATransition[] finalTr=aut.getTransition();
		int removed=0;
		for (int ind=0;ind<finalTr.length;ind++)
		{
			...
			{
				if(amIReachable(fs[i],aut,arr,new int[aut.sumStates()][],0)) //if final state fs[i] is reachable from arrival state arr
					remove = false;
			}
