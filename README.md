...
The following snippet imports a product line (i.e., a family) either in the textual format `.prod`, 
as a `.xml` FeatureIDE model (whose products are generated by FeatureIDE), or as a `.dimac` file 
(products are generated when importing). 
Dimac importer is set to only generate maximal products.
Different FMCA are instantiated using different constructors.
Afterwards, two orchestrations of the product line are computed.
