...
aut.add(bdc.importDATA(dir+"BusinessClient.mxe.data"));//loading textual .data description of a CA
aut.add(bdc.importDATA(dir+"Hotel.mxe.data"));
...
```java
MSCA aut = new BasicMxeConverter().importMxe(dir+"(BusinessClientxHotelxEconomyClient).mxe");		
Product p = new Product(new String[] {"card","sharedBathroom"}, new String[] {"singleRoom"});
MSCA orc = new ProductOrchestrationSynthesisFunction(new Agreement(),p).apply(aut);	
```
